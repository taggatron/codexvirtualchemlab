...
try {
  ...
  assert(quadratInitial.random_quadrat_sampling_practical.environment.realistic_layered_trees, 'Layered realistic trees are missing.');
  assert(quadratInitial.random_quadrat_sampling_practical.environment.tree_depth_rows === 3, 'Tree depth layering is incorrect.');
  assert(quadratInitial.random_quadrat_sampling_practical.environment.curved_tapered_trunks && quadratInitial.random_quadrat_sampling_practical.environment.radial_connected_branches, 'Curved trunks or connected radial branches are missing.');
  assert(quadratInitial.random_quadrat_sampling_practical.environment.visible_root_flares && quadratInitial.random_quadrat_sampling_practical.animation.upper_tree_canopies_sway_from_fixed_lower_trunks, 'Tree grounding or upper-canopy wind flex is missing.');
  ...
  assert(shoreInitial.rocky_shore_transect_practical.landscape.realistic_eroded_cliff, 'Realistic eroded cliff detail is missing.');
  ...
  assert(shoreInitial.rocky_shore_transect_practical.landscape.broken_projecting_rock_ledges === 19 && shoreInitial.rocky_shore_transect_practical.landscape.recessed_branched_fissures, 'Broken cliff strata or branching fissures are missing.');
  assert(shoreInitial.rocky_shore_transect_practical.landscape.rock_beach_floor_extends_beyond_visible_view, 'Rock beach floor overscan is missing.');
  assert(shoreInitial.rocky_shore_transect_practical.landscape.rock_beach_floor_bounds_world.x[0] <= -13.5 && shoreInitial.rocky_shore_transect_practical.landscape.rock_beach_floor_bounds_world.x[1] >= 13.5, 'Rock beach lateral extent is insufficient.');
  ...
  assert(JSON.stringify(shoreInitial.rocky_shore_transect_practical.tide.water_world_dimensions) === JSON.stringify([22, 12]) && shoreInitial.rocky_shore_transect_practical.tide.foam_world_span === 22.4, 'Tide or foam overscan is insufficient.');
