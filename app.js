...
function draw(skipWebGL = false) { if (state.selected !== lastSelectedPractical) { state.reaction = null; lastSelectedPractical = state.selected } regions = []; ctx.clearRect(0, 0, W, H); header(); sidebar(); main(); rightbar(); if (state.dose) drawDosePanel(); visibleCtx.save(); visibleCtx.setTransform(1, 0, 0, 1, 0, 0); visibleCtx.globalCompositeOperation = 'copy'; visibleCtx.drawImage(buffer, 0, 0); visibleCtx.restore(); if (!state.dose && !skipWebGL && !state.drag) lab3d.render(performance.now(), state, practicals[state.selected]) }
