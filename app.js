...
function drawQuadratViewControls(x, w, arenaTop) {
  ...
  if (birdsEye) {
    const gridW = 128, gridX = viewX - gridW - gap, gridY = viewY, gridOn = !!state.quadratShowGrid;
    rr(gridX, gridY, gridW, btnH, 8, gridOn ? C.teal : 'rgba(10,26,20,.62)', gridOn ? C.teal : 'rgba(255,255,255,.32)');
    text(gridOn ? '▦ GRID ON' : '▦ GRID OFF', gridX + gridW / 2, gridY + btnH / 2, 9.6, '#fff', 800, 'center');
    hit('toggle-quadrat-grid', gridX, gridY, gridW, btnH);
  }
}
...
function drawApparatus(id, cx, cy, w, benchY, arenaX) {
  ...
  // WebGL loads reliably in practice, so no practical falls back to the
  // legacy 2D acid-flask pour animation any more.
...
canvas.addEventListener('pointerdown', e => {
  ...
  else if (r.id === 'toggle-quadrat-view') { state.quadratCameraView = state.quadratCameraView === 'birdseye' ? 'perspective' : 'birdseye'; state.toast = state.quadratCameraView === 'birdseye' ? 'Bird\'s-eye view: looking straight down over the sampling grid.' : 'Perspective view restored.'; draw(); }
  else if (r.id === 'toggle-quadrat-grid') { state.quadratShowGrid = !state.quadratShowGrid; draw(); }
