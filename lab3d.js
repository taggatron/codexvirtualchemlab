...
export class LabRenderer3D{
  ...
  flameTestRig(state){
    ...
    const additive=(opacity=0)=>new THREE.MeshBasicMaterial({color:salt.flameColor,transparent:true,opacity,blending:THREE.AdditiveBlending,depthWrite:false,toneMapped:false,side:THREE.DoubleSide}),outerMat=additive(),coreMat=additive(),haloMat=additive(),outer=new THREE.Mesh(new THREE.ConeGeometry(.48,1.55,64,1,true),outerMat),core=new THREE.Mesh(new THREE.ConeGeometry(.27,1.26,56,1,true),coreMat),halo=new THREE.Mesh(new THREE.SphereGeometry(.6,48,28),haloMat);outer.position.set(0,2.08,.6);core.position.set(0,1.98,.61);halo.position.set(0,2.07,.61);halo.scale.y=1.28;[outer,core,halo].forEach(mesh=>{mesh.visible=false;mesh.renderOrder=12;mesh.castShadow=false;g.add(mesh)});const colourLight=new THREE.PointLight(salt.flameColor,0,4.8,1.7);colourLight.position.set(0,2.05,.72);g.add(colourLight);
    this.dynamic.push({kind:'flameTest',spatula,saltLoad,jarPoint:new THREE.Vector3(jarXs[selected],1.02,jarZ),restPoint:new THREE.Vector3(1.2,.24,2.35),flamePoint:new THREE.Vector3(-.05,2.08,.6),outer,core,halo,outerMat,coreMat,haloMat,colourLight,seed:selected*1.7+.4});
